    ...
    def run(self):
        ...
        try:
            ...
            self.log_msg.emit(f"  {len(basins)} basin(s), total area "
                              ...
                              f"{sum(b.area for b in basins):.2f} km2")
            ...
            self.log_msg.emit(
                ...
                f"({n_steps_storm} steps x {p['dt']:.2f} hr)"
            ...
            self.log_msg.emit(f"\nOutlet peak flow: {outlet_peak:.2f} m3/s")
    ...
    def run(self):
        ...
        try:
            ...
            for name, q in res['hydros'].items():
                ...
                diff_str = f"  (RORB {rorb_peak:.3f}, ??{diff:+.1f}%)" if diff is not None else ""


                self.log_msg.emit(f"  {name}: peak = {eng_peak:.3f} m3/s{diff_str}")
...
class RorbModelDialog(QDialog):
    ...
    def _tab_network(self):
        ...
        for lbl in (self._net_reaches_lbl, self._net_junctions_lbl,
            ...
            lbl.setStyleSheet(
                ...
                "padding: 3px 8px; background: #e8f0fe; border: 1px solid #aac; "
        ...
        export_btn.clicked.connect(self._export_network_csv)
    ...
    def _node_type_label(self, node):
        ...
        if isinstance(node, Confluence):
            ...
            return "Outlet" if node.isOut else "Confluence"
    ...
    def _node_type_color(self, label):
        ...
        return {'Outlet': '#f97316', 'Confluence': '#3b82f6',


                'Sub-area': '#22c55e'}.get(label, '#888')
    ...
    def _build_network_table(self):
        ...
        lyr_r = self._reach_row.layer()


        lyr_c = self._cent_row.layer()


        lyr_j = self._junc_row.layer()


        lyr_b = self._basin_row.layer()





        if not lyr_r or not lyr_j:


            self._net_warn.setText("Select at least a Reach layer and Confluences layer.")
        ...
        try:
            ...
            confluences = build_confluences(lyr_j,
            ...
            basins = build_basins(lyr_c, lyr_b,


                                  self._cent_row.field("id"),


                                  self._cent_row.field("fi")) if lyr_c and lyr_b else []
            ...
            for j, reach in enumerate(catchment._edges):
                ...
                for i in range(nv):
                    ...
                    if ds != sentinel:
                        ...
                        up_node = catchment._vertices[i]


                        dn_node = catchment._vertices[ds]
                ...
                len_km = reach.length() / 1000.0


                total_len += len_km


                up_label = self._node_type_label(up_node) if up_node else "???"


                dn_label = self._node_type_label(dn_node) if dn_node else "???"
                ...
                if up_node is None or dn_node is None:
                    ...
                    warnings.append(f"Reach '{reach.name}': not connected (check snap tolerance).")
                ...
                rows_data.append({
                    ...
                    'from':     up_node.name if up_node else "?",


                    'from_type': up_label,


                    'to':       dn_node.name if dn_node else "?",
            ...
            for i, r in enumerate(rows_data):
                ...
                ft_item = QTableWidgetItem(r['from_type'])


                ft_item.setForeground(QColor(self._node_type_color(r['from_type'])))


                self._net_table.setItem(i, 2, ft_item)
                ...
                tt_item = QTableWidgetItem(r['to_type'])


                tt_item.setForeground(QColor(self._node_type_color(r['to_type'])))


                self._net_table.setItem(i, 4, tt_item)





                len_item = QTableWidgetItem(f"{r['len_km']:.3f}")


                len_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)


                self._net_table.setItem(i, 5, len_item)
            ...
            n_junc = sum(1 for v in catchment._vertices if isinstance(v, Confluence))


            n_bas  = sum(1 for v in catchment._vertices if isinstance(v, Basin))


            self._net_reaches_lbl.setText(f"Reaches: {len(reaches)}")


            self._net_junctions_lbl.setText(f"Confluences: {n_junc}")


            self._net_basins_lbl.setText(f"Sub-areas: {n_bas}")
    ...
    def _export_network_csv(self):
        ...
        if self._net_table.rowCount() == 0:
            ...
            QMessageBox.warning(self, "Network", "Build the network first.")
        ...
        path, _ = QFileDialog.getSaveFileName(
            ...
            self, "Export network", "", "CSV files (*.csv)")
        ...
        headers = ["Reach", "From node", "From type", "To node", "To type",
        ...
        with open(path, 'w', newline='') as f:
            ...
            for row in range(self._net_table.rowCount()):
                ...
                w.writerow([
                    ...
                    self._net_table.item(row, col).text()
        ...
        QMessageBox.information(self, "Network", f"Exported to:\n{path}")
    ...
    def _tab_storm(self):
        ...
        note = QLabel(
            ...
            "Workflow: IFD depth x ARF x temporal pattern % ??? rainfall series ??? "
            ...
            if patterns:
                ...
                if incs:
                    ...
                    lines += [
                        ...
                        f"Temporal pattern: {n_steps} steps x {dt_min} min (dt={dt_hr:.4f} hr)",
    ...
    def _storm_generate_and_run(self):
        ...
        if errors:
            ...
            self._tabs.setCurrentIndex(6)   # Results tab
    ...
    def _tab_results(self):
        ...
        self._table.setHorizontalHeaderLabels(
            ...
            ["Node", "Type", "Peak (m3/s)", "RORB (m3/s)", "Time to Peak (hr)"])
    ...
    def _plot(self, results):
        ...
        if source == 'files':
            ...
            if sel_type == 'reach' and sel_val is not None:
                ...
                if reach:
                    ...
                    self._ax.set_title(
                        ...
                        f"k = kc x kr = {results['kc']} x {reach['kr']:.3f} = {reach['k']:.3f}   "


                        f"peak = {peak:.3f} m3/s  at {ttp:.2f} hr",
            ...
            if r:
                ...
                self._ax.set_title(f"{r['name']}  Peak = {r['peak_flow']:.2f} m3/s")
