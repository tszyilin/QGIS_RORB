...
from qgis.PyQt.QtWidgets import (
    ...
    QCheckBox, QMessageBox, QSizePolicy, QFrame,
)
...
from .compat import (
    FieldTypeInt, FieldTypeString, FieldTypeDouble,
    WkbLineGeometry,
    LeftButton, RightButton, Key_Escape,
    FrameHLine, FrameSunken,
    DialogAccepted,
)
...
_CENTROID_FIELDS = [
    ('id',         FieldTypeInt,    'Node ID'),
    ('name',       FieldTypeString, 'Sub-area name'),
    ('area_km2',   FieldTypeDouble, 'Area (km²)'),
    ('fi',         FieldTypeDouble, 'Fraction impervious'),
    ('print_node', FieldTypeInt,    'Print at node (0/1)'),
]

_JUNCTION_FIELDS = [
    ('id',        FieldTypeInt,    'Junction ID'),
    ('name',      FieldTypeString, 'Junction name'),
    ('is_outlet', FieldTypeInt,    'Is outlet (0/1)'),
]

_REACH_FIELDS = [
    ('id',          FieldTypeInt,    'Reach ID'),
    ('name',        FieldTypeString, 'Reach name'),
    ('type',        FieldTypeInt,    'Reach type (1–4)'),
    ('length_km',   FieldTypeDouble, 'Length (km)'),
    ('slope',       FieldTypeDouble, 'Slope (%)'),
    ('translation', FieldTypeInt,    'Translation (time incs, 0=off)'),
    ('print_ud',    FieldTypeInt,    'Print U/S and D/S (0/1)'),
]
...
class _LineTool(QgsMapTool):
    ...
    def __init__(self, canvas):
        ...
        self._pts = []
        self._rb  = QgsRubberBand(canvas, WkbLineGeometry)
    ...
    def _reset(self):
        self._pts = []
        self._rb.reset(WkbLineGeometry)
...
def _hsep():
    line = QFrame()
    line.setFrameShape(FrameHLine)
    line.setFrameShadow(FrameSunken)
    return line
