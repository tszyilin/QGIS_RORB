"""
RORB Results Viewer — standalone desktop application.
Adapted from the QGIS plugin (rorb_qgis/results_dialog.py) with all
QGIS dependencies removed.  Requires only PyQt5, matplotlib, and numpy.
...
from PyQt5.QtWidgets import (
    QMainWindow, QDialog, QVBoxLayout, QHBoxLayout, QFormLayout, QGroupBox,
    QLabel, QPushButton, QTabWidget, QWidget,
    QComboBox, QTableWidget, QTableWidgetItem,
    QHeaderView, QFileDialog, QMessageBox,
    QLineEdit, QCheckBox, QScrollArea,
    QProgressBar, QSplitter, QInputDialog,
    QRadioButton, QButtonGroup, QAbstractItemView, QMenu, QAction,
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QUrl
from PyQt5.QtGui import QFont, QColor, QDesktopServices

# ── PyQt5 constants (mirrors compat.py) ───────────────────────────────────────
AlignRightVCenter  = int(Qt.AlignRight | Qt.AlignVCenter)
...
# ── Settings persistence (JSON in home dir) ────────────────────────────────────
_SETTINGS_FILE = Path.home() / '.rorb_results_viewer.json'
...
class _ScanWorker(QThread):
    ...
    def run(self):
        try:
            from engine import parse_out_hydrograph, parse_out_rainfall, parse_out_ttp, parse_stm
...
class RorbResultsViewer(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def _add_extra_event(self):
        ...
        entry = next(
            (e for e in self._entries_for_aep(aep, scen)
             if e['parsed'][2] == dur_min and e['parsed'][3] == tp_num), None)
        if entry is None:
    ...
    def _refresh_preview(self):
        ...
        for source, aep, dur_label, tp_num, entry in self._get_export_events():
            ...
            for col, text in [(1, aep), (2, dur_label)]:
                it = QTableWidgetItem(text); it.setFlags(ItemIsEnabled)
                self._exp_preview_table.setItem(row, col, it)
            tp_it = QTableWidgetItem(f"TP{tp_num}"); tp_it.setFlags(ItemIsEnabled)
            tp_it.setTextAlignment(AlignCenter)
        ...
        if has_shifted:
            self._exp_version_warn.setText(
                ...
                "corrected by one time step so that it starts at 0.00 h.")
    ...
    def _save_state(self):
        try:
            ...
            data = {
                ...
                'rep_method':    self._rep_method.currentData() or 'closest',
                'export_folder': self._exp_folder_edit.text(),
            }
    ...
    def _restore_state(self):
        ...
        rep = data.get('rep_method', 'closest')
    ...
    def _rename_scenario(self):
        ...
        if self._active == old: self._active = new
    ...
    def _clear_all_scenarios(self):
        self._scenarios.clear(); self._scenario_folders.clear(); self._active = None
        self._scen_combo.blockSignals(True); self._scen_combo.clear()
        self._scen_combo.blockSignals(False)
        self._refresh_all(); self._save_state()
    ...
    def _on_scan_done(self, scenario_name, files):
        ...
        self._scan_status.setText(f"'{scenario_name}': {ok}/{len(files)} files OK")
        self._refresh_all(); self._save_state()
    ...
    def _refresh_combos(self):
        ...
        self._crit_node_combo.blockSignals(True); self._crit_node_combo.clear()
        for n in all_nodes: self._crit_node_combo.addItem(n)
    ...
    def _active_files(self, scenario=None):
        return self._scenarios.get(scenario or self._active, {})
    ...
    def _compute_critical(self, aep, node, scenario=None):
        ...
        if method == 'above':
            ...
            rep_tp_num, rep_peak, rep_entry = min(
                pool, key=lambda x: x[1] - mean_peak if above else abs(x[1] - mean_peak))
        else:
            rep_tp_num, rep_peak, rep_entry = min(
    ...
    def _refresh_viewer(self):
        ...
        self._node_combo_v.blockSignals(True); self._node_combo_v.clear()
    ...
    def _replot(self):
        ...
        # Representative TP for the currently displayed duration
        rep_tp = None
        rep_tp_peak = None
        if crit:
    ...
    def _export_hydros(self):
        ...
        for source, aep, dur_label, tp_num, entry in events:
            ...
            if t:
                ...
                if t[0] > 0: t = [0.0] + t; q = [0.0] + q
                t.append(float(t[-1]) + dt); q.append(0.0)
        ...
        if adjusted:
            ...
            msg += (f"\n\nNote: {len(adjusted)} file(s) from RORB <v6.52 had their time axis "
                    f"shifted back by one time step:\n" + file_lines)
